...
class DataAugment:
    ...
    def random_panning(self, img, min_translation, max_translation):
...
class TrainDataPackage:
    ...
    def random_patch(self, image, num):
        ...
        for n in range(num):
            ...
            if p <= n < 2 * p:
                _, patch = self.aug.random_panning(patch, (-0.2, -0.2), (0.2, 0.2))
            if 2 * p <= n < 3 * p:
                _, patch = self.aug.random_rotate(patch, (0., 1.))
            if 3 * p <= n < 4 * p:
                _, patch = self.aug.random_scale(patch, (1., 1.), (2., 2.))
            if 4 * p <= n < 5 * p:
                _, patch = self.aug.random_shear(patch, (0., 0.2))

            for oc in cv2.split(patch):
                patch = np.array(oc, dtype=np.float32) / 255.
...
def train_loader(config):
    ...
    dst = torch.utils.data.DataLoader(train_dataset, batch_size=config.batch_size, drop_last=True,
